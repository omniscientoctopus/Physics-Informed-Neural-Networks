...
# #storing gradients of weights and biases together
...
# #     obj_text_b = codecs.open("Non_Stiff_problem/Gradients/gradients_f" + str(i+1) + ".json", 'r', encoding='utf-8').read()
...
# #     obj_text_b = codecs.open("Non_Stiff_problem/Gradients/gradients_u" + str(i+1) + ".json", 'r', encoding='utf-8').read()
...
#     dataset = pd.DataFrame({'PDE residual (non-stiff)': f_1['f'+ str(i)], 
#                             'Boundary residual (non-stiff)': u_1['u'+ str(i)]})
# #                             'PDE residual (stiff)': np.abs(f_2['f'+ str(i)]), 
# #                             'Boundary residual (stiff)': np.abs(u_2['u'+ str(i)])})
...
#     sns.displot(dataset, kind='kde', log_scale = (False,False))
